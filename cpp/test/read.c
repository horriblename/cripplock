...
const gchar *colors[] = {"black",   "orchid", "fuchsia", "indigo",
                         "thistle", "sienna", "azure",   "plum",
                         "lime",    "navy",   "maroon",  "burlywood"};

static GtkPadActionEntry pad_actions[] = {
    {GTK_PAD_ACTION_BUTTON, 1, -1, N_("Nuclear strike"), "pad.nuke"},
    {GTK_PAD_ACTION_BUTTON, 2, -1, N_("Release siberian methane reserves"),
     "pad.heat"},
    ...
    {GTK_PAD_ACTION_BUTTON, 6, -1, N_("Lart whoever asks about this button"),
     "pad.lart"},
    {GTK_PAD_ACTION_RING, -1, -1, N_("Earth axial tilt"), "pad.tilt"},
    {GTK_PAD_ACTION_STRIP, -1, -1, N_("Extent of weak nuclear force"),
     "pad.dissolve"},
};
...
static AxesInfo *axes_info_new(void) {
  AxesInfo *info;
...
static EventData *event_data_new(void) {
  EventData *data;
...
static void update_axes_from_event(GdkEvent *event, EventData *data) {
...
static void render_arrow(cairo_t *cr, gdouble x_diff, gdouble y_diff,
                         const gchar *label) {
  cairo_save(cr);
...
static void draw_axes_info(cairo_t *cr, AxesInfo *info,
  ...
  if (axes & GDK_AXIS_FLAG_XTILT && axes & GDK_AXIS_FLAG_YTILT) {
    gdk_device_get_axis(info->last_source, info->axes, GDK_AXIS_XTILT, &tilt_x);
    gdk_device_get_axis(info->last_source, info->axes, GDK_AXIS_YTILT, &tilt_y);
  ...
  if (axes & GDK_AXIS_FLAG_WHEEL) {
    gdk_device_get_axis(info->last_source, info->axes, GDK_AXIS_WHEEL, &wheel);
...
static const gchar *tool_type_to_string(GdkDeviceToolType tool_type) {
  switch (tool_type) {
...
static void draw_device_info(GtkWidget *widget, cairo_t *cr,
                             GdkEventSequence *sequence, gint *y,
                             AxesInfo *info) {
  PangoLayout *layout;
  GString *string;
  gint height;
  ...
  if (info->last_tool) {
    ...
    tool_type =
        tool_type_to_string(gdk_device_tool_get_tool_type(info->last_tool));
...
static void update_label_text(GtkWidget *label, const gchar *text) {
  gchar *markup = NULL;
...
static gboolean reset_label_text_timeout_cb(gpointer user_data) {
...
static void update_label_and_timeout(GtkWidget *label, const gchar *text) {
  if (pad_action_timeout_id)
    g_source_remove(pad_action_timeout_id);

  update_label_text(label, text);
  pad_action_timeout_id =
      g_timeout_add(200, reset_label_text_timeout_cb, label);
}
...
static void init_pad_controller(GtkWidget *window, GtkWidget *label) {
  ...
  pad_controller = gtk_pad_controller_new(GTK_WINDOW(window),
                                          G_ACTION_GROUP(action_group), NULL);
  ...
  for (i = 0; i < G_N_ELEMENTS(pad_actions); i++) {
    ...
    g_signal_connect(action, "activate", G_CALLBACK(on_action_activate), label);
  ...
  g_object_set_data_full(G_OBJECT(window), "pad-controller", pad_controller,
                         g_object_unref);

  g_object_unref(action_group);
}

GtkWidget *do_event_axes(GtkWidget *toplevel) {
  ...
  if (!window) {
    ...
    gtk_widget_add_events(box, GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK |
                                   ...
                                   GDK_LEAVE_NOTIFY_MASK | GDK_TOUCH_MASK);

    event_data = event_data_new();
    g_object_set_data_full(G_OBJECT(box), "gtk-demo-event-data", event_data,
                           (GDestroyNotify)event_data_free);

    g_signal_connect(box, "event", G_CALLBACK(event_cb), event_data);
    g_signal_connect(box, "draw", G_CALLBACK(draw_cb), event_data);
